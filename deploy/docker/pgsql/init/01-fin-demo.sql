...
COMMENT ON TABLE fin_portfolio_holding IS '财经演示-组合持仓';
COMMENT ON COLUMN fin_portfolio_holding.id IS '主键ID';
COMMENT ON COLUMN fin_portfolio_holding.region IS '境内/境外';
COMMENT ON COLUMN fin_portfolio_holding.product_type IS '产品类型';
COMMENT ON COLUMN fin_portfolio_holding.product_code IS '产品代码';
COMMENT ON COLUMN fin_portfolio_holding.trade_date IS '交易日期';
COMMENT ON COLUMN fin_portfolio_holding.market_value IS '市值';
COMMENT ON COLUMN fin_portfolio_holding.cost_basis IS '成本';
COMMENT ON COLUMN fin_portfolio_holding.is_stressed IS '是否压力情景';
COMMENT ON COLUMN fin_portfolio_holding.strategy_tags IS '策略标签（逗号分隔）';
